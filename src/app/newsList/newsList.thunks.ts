import { Action, Dispatch, AnyAction } from 'redux';
import { ThunkAction, ThunkDispatch } from 'redux-thunk';
import { AppState } from '../redux/root.reducer';
import { Story } from '../model/story';
import { newStoriesLoaded, newStoryIdsLoaded, previewStory } from './newsList.action';
import { selectLastStoryLoadedIndex, selectStoryIds } from './newsList.selectors';

const STORIES_URL = 'https://hacker-news.firebaseio.com/v0/newstories.json';
const ITEM_URL = 'https://hacker-news.firebaseio.com/v0/item/{itemId}.json';
const PAGE_SIZE = 40;

export const loadNewStoryIds = () => {
    return async (dispatch: any) => {
        return new Promise<void>(async (resolve) => {
            try {
                const response: any = await fetch(STORIES_URL);
                const storyIds = await response.json();

                dispatch(newStoryIdsLoaded(storyIds));
            }
            catch (err) {
                //TODO show error;
                console.error('something bad happened! ', err)
            }
            resolve();
        });
    };
};

export const loadStoriesItems = () => {
    return async (dispatch: any, getState: () => AppState) => {
        return new Promise<void>(async (resolve) => {
            const appState = getState();
            const lastStoryLoaded = selectLastStoryLoadedIndex(appState);
            const storyIds = selectStoryIds(appState);

            const maxIndex = storyIds.length > PAGE_SIZE ? PAGE_SIZE : storyIds.length;

            const storiesPromises: Promise<Story>[] = [...storyIds]
                .splice(lastStoryLoaded, maxIndex)
                .map(async (id: number) => {
                    return fetch(ITEM_URL.replace('{itemId}', id.toString()))
                        .then((response) => response.json());
                });

            Promise.all(storiesPromises)
                .then((stories) => {
                    if (lastStoryLoaded === 0) {
                        dispatch(previewStory(stories[0]));
                    }
                    dispatch(newStoriesLoaded(stories));
                })
                .catch((err) => {
                    //TODO handle error
                    console.error('something bad happened! ', err);
                });

            resolve();
        });
    };
};